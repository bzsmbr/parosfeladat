...
public class JuryService(AppDbContext dbContext) : IJuryService
{
    ...
    {
        bool exists = await dbContext.Juries.AnyAsync(x => x.PhoneNumber == model.PhoneNumber.Value &&
                                                           x.Email.ToLower() == model.Email.Value.ToLower() &&
                                                           x.Name.ToLower() == model.Name.Value.ToLower());
        if (exists)
        {
            return Error.Conflict(description: "This jury already exists!");
        }
